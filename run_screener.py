...
def recalculate_scores(df, weight_quality, weight_value, threshold_buy, threshold_monitor,
    ...
    def decide(row):
        ...
        # BUT: Block if revenue declining (deteriorating business)
        revenue_growth = row.get('revenue_growth_3y')

        if composite >= 85:  # Raised from 80 to 85 (more selective)
            # Check for revenue decline (works for both value and growth)
        ...
        # BUT: Block if revenue declining (works for both value and growth)
